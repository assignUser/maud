...
- indicate that optional features are disabled, for example with
  `MAUD_DISABLED_MODULES`
...
https://timsong-cpp.github.io/cppwp/n4868/module.unit#3), and if you have
written partitions then you probably don't have anything in the primary
module interface unit except those `export import`s. This feels boilerplate-y,
so if no primary module interface unit is detected then one will be generated
which just does those `export import`s.
...
By default, maud uses a custom module scanner which ignores preprocessing for
efficiency and stops reading source files after the import declarations.
This works in the most common case where the preprocessor is only
used to populate the global module fragment with `#include`s and an occasional
`#define`, which leaves the module dependency graph unaffected. However it is
possible for the preprocessor to affect module and import declarations.
For example:
- an import declaration could be inside a conditional preprocessing block:
  ...
  #endif
  ```
- a set of import declarations could be `#include`d:
  ```c++
  module foo;
...
https://timsong-cpp.github.io/cppwp/n4868/module.global.frag#1),
...
as described in [P3034R1](https://isocpp.org/files/papers/P3034R1.html).
...
`${CMAKE_SOURCE_DIR}/some/dir/f.txt.in2` will be rendered to
...
For additional syntactic sugar in the common case of slightly modifying a
